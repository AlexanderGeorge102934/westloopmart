...
class PostController extends GetxController {
  ...
  final InitialPostsRepository _initialPostsRepository = InitialPostsRepository();
  ...
  Future<void> postOffer() async {
    final user = await _initialPostsRepository.getCurrentUser();
    if (user == null) {
    ...
    try {
      ...
      /// Get image urls
      List<String> imageUrls = await _initialPostsRepository.uploadImages(images);

      /// Get user's position
      final position = await _initialPostsRepository.determinePosition();
      ...
      await _initialPostsRepository.addInitialPost(post);
